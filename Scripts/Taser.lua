...
function taser.client_onCreate( self )
...
function taser.client_onRefresh( self )
	self:loadAnimations()
end

function taser.loadAnimations( self )
...
function taser.client_onUpdate( self, dt )
...
function taser.client_onEquip( self, animate )
	...
	if self.tool:isLocal() then
		-- Sets taser renderable, change this to change the mesh
		self.tool:setFpRenderables( currentRenderablesFp )
		swapFpAnimation( self.fpAnimations, "unequip", "equip", 0.2 )
	end
end

function taser.client_onUnequip( self, animate )
...
function taser.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function taser.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function taser.onAim( self, aiming )
	self.aiming = aiming
...
function taser.sv_n_onShoot( self, dir )
	self.network:sendToClients( "cl_n_onShoot", dir )
end

function taser.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function taser.onShoot( self, dir )
...
function taser.calculateFirePosition( self )
...
function taser.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function taser.calculateFpMuzzlePos( self )
...
function taser.cl_onPrimaryUse( self, state )
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend( sm.localPlayer.getInventory(), obj_plantables_potato, 1 ) then
			...
			if owner then
				local result, raycastResult = sm.localPlayer.getRaycast( 50/4 )
                if raycastResult.type == "character" then
                    self.network:sendToServer("sv_stunCharacter",{raycastResult:getCharacter(),raycastResult.pointWorld,owner})
                end
            end

			-- Timers
			self.fireCooldownTimer = 4
...
function taser.sv_stunCharacter(self,data)
	local character = data[1]
	local position = data[2]
	local owner = data[3]
	if owner ~= hostPlayer then
		self.network:sendToClient(hostPlayer,"cl_sendTextMessage",tostring("#00FF00-LOG-#FFFFFF "..owner.name.." tased "..character:getPlayer().name.." at "..math.floor(position.x*4).." "..math.floor(position.y*4).." "..math.floor(position.z*4)))
	end
	self.network:sendToClients("cl_taseEffect",data)
	local randomDirection = sm.vec3.new(math.random(500)-250,math.random(500)-250,500)
	character:applyTumblingImpulse( randomDirection )
...
function taser.cl_taseEffect(self,data)
	local character = data[1]
	local position = data[2]
	self.taseEffect = sm.effect.createEffect( "PowerSocket - Activate" )
	self.taseEffect:setPosition( position )
	self.taseEffect:start()
end

function taser.cl_onSecondaryUse( self, state )
...
function taser.client_onEquippedUpdate( self, primaryState, secondaryState )
...
function taser.cl_sendTextMessage(self,text)
